...
std::vector<HandlingManager::HandlingAttribEntry> HandlingManager::ParseAttribEntries(uint8_t count, RakNet::BitStream* bs)
{
	...
	if (!bs)
		return entries;
	if (count > 64) {
		ClientLog(LogLevel::Error, std::format("ParseAttribEntries: Rejecting excess count {} (max 64)", count));
		return {};
	}
	...
	for (int i = 0; i < count; ++i) {
		if (bs->GetNumberOfUnreadBits() < 16) {
			ClientLog(LogLevel::Error, std::format("ParseAttribEntries: Truncated bitstream at entry {} of {}", i, count));
			return {};
		}
		CHandlingAttrib attrib;
		if (!bs->Read(attrib)) {
			ClientLog(LogLevel::Error, std::format("ParseAttribEntries: Failed reading attrib at index {}", i));
			return {};
		}
		uint8_t typeByte;
		if (!bs->Read(typeByte)) {
			ClientLog(LogLevel::Error, std::format("ParseAttribEntries: Failed reading typeByte for attrib {}", static_cast<int>(attrib)));
			return {};
		}
		...
		case TYPE_FLOAT: {
			float v;
			if (!bs->Read(v)) {
				ClientLog(LogLevel::Error, std::format("ParseAttribEntries: Failed reading float for attrib {}", static_cast<int>(attrib)));
				return {};
			}
			entry.value.f = v;
			ClientLog(LogLevel::Debug, std::format("ParseAttribEntries: Entry {}: attrib={} (FLOAT), val={:.4f}", i, static_cast<int>(attrib), v));
			break;
		}
		case TYPE_UINT:
		case TYPE_FLAG: {
			uint32_t v;
			if (!bs->Read(v)) {
				ClientLog(LogLevel::Error, std::format("ParseAttribEntries: Failed reading uint for attrib {}", static_cast<int>(attrib)));
				return {};
			}
			entry.value.u = v;
			ClientLog(LogLevel::Debug, std::format("ParseAttribEntries: Entry {}: attrib={} (UINT/FLAG), val={:#x} ({})", i, static_cast<int>(attrib), v, v));
			break;
		}
		case TYPE_BYTE: {
			uint8_t v;
			if (!bs->Read(v)) {
				ClientLog(LogLevel::Error, std::format("ParseAttribEntries: Failed reading byte for attrib {}", static_cast<int>(attrib)));
				return {};
			}
			entry.value.b = v;
			ClientLog(LogLevel::Debug, std::format("ParseAttribEntries: Entry {}: attrib={} (BYTE), val={}", i, static_cast<int>(attrib), static_cast<int>(v)));
			break;
		}
		default:
			ClientLog(LogLevel::Warning, std::format("ParseAttribEntries: Unknown expectedType for attrib {}", static_cast<int>(attrib)));
			break;
		}

		if (!CanSetHandlingAttrib(attrib)) {
			ClientLog(LogLevel::Debug, std::format("ParseAttribEntries: Attrib {} is read-only, skipping", static_cast<int>(attrib)));
			continue;
		}
	...
	if (!m_pendingVehicleAttribs.empty()) {
		for (auto it = m_pendingVehicleAttribs.begin(); it != m_pendingVehicleAttribs.end();) {
			...
			if (IsVehiclePointerValid(gtaVeh) && gtaVeh->m_pHandlingData) {
				...
				ClientLog(LogLevel::Info, std::format("ProcessPendingCommands: Vehicle {} is now available, applying {} queued attribs", targetVehId, attribs.size()));
...
void HandlingManager::OnVehicleStreamIn(CVehicle* pVehicle, uint16_t sampId)
{
	...
	if (doorIt != m_vehicleDoorStates.end() && doorIt->second != 0) {
		...
		ClientLog(LogLevel::Info, std::format("OnVehicleStreamIn: Applied {} cached missing door(s) for vehicle {}", appliedCount, sampId));
	}
	...
	if (pendingIt != m_pendingVehicleAttribs.end()) {
		...
		ClientLog(LogLevel::Info, std::format("OnVehicleStreamIn: Applied pending handling for vehicle {}", sampId));
		return;
	}
...
void HandlingManager::ProcessVehicleMods(uint16_t sampVehicleId, const std::vector<HandlingAttribEntry>& entries)
{
	...
	if (!IsVehiclePointerValid(gtaVehicle) || !gtaVehicle->m_pHandlingData) {
		...
		ClientLog(LogLevel::Info, std::format("ProcessVehicleMods: Vehicle {} not in pool, queued {} attribs", sampVehicleId, entries.size()));
		return;
	}
...
void HandlingManager::ProcessVehicleDoorState(uint16_t sampVehicleId, uint8_t doorId, bool missing)
{
	...
	if (IsVehiclePointerValid(gtaVehicle)) {
		...
		ClientLog(LogLevel::Info, std::format("ProcessVehicleDoorState: Applied doorId {} (missing={}) to vehicleId {}", doorId, missing, sampVehicleId));
	} else {
		ClientLog(LogLevel::Info, std::format("ProcessVehicleDoorState: vehicleId {} not in pool, cached doorId {} (missing={})", sampVehicleId, doorId, missing));
	}
}
...
void HandlingManager::ProcessModelMods(uint16_t modelId, const std::vector<HandlingAttribEntry>& entries)
{
	...
	ClientLog(LogLevel::Info, std::format("ProcessModelMods: Applied {} attribs to model {}", entries.size(), modelId));
}
...
void HandlingManager::SendHandlingPacket(CustomVehAction action, RakNet::BitStream* bs)
{
	...
	ClientLog(LogLevel::Debug, std::format("SendHandlingPacket: action={}, bytes={}, sent={}", static_cast<int>(action), packet.GetNumberOfBytesUsed(), sent));
}
...
void HandlingManager::SendInitPacket()
{
	if (m_initSent.exchange(true, std::memory_order_acq_rel)) {
		ClientLog(LogLevel::Debug, "ACTION_INIT already sent for current session.");
		return;
	}

	ClientLog(LogLevel::Info, std::format("Sending ACTION_INIT packet (compat_ver=0x{:X})...", EXTENDEDVEH_COMPAT_VERSION));
	RakNet::BitStream bs;
...
bool HandlingManager::ProcessAction(CustomVehAction action, RakNet::BitStream* bs)
{
	...
	ClientLog(LogLevel::Debug, std::format("ProcessAction: action={}", static_cast<int>(action)));
	...
	case ACTION_INIT_RESPONSE: {
		...
		if (!bs->Read(compat_ver) || !bs->Read(allowed)) {
			ClientLog(LogLevel::Error, "ACTION_INIT_RESPONSE: Failed to read packet data");
			return false;
		}

		ClientLog(LogLevel::Info, std::format("ACTION_INIT_RESPONSE: allowed={}, server_compat_ver=0x{:X}", allowed, compat_ver));
		if (allowed && compat_ver == EXTENDEDVEH_COMPAT_VERSION) {
			m_isServerAuthorized.store(true, std::memory_order_release);
			ClientLog(LogLevel::Info, "authorization established.");
	...
	case ACTION_SET_VEHICLE_HANDLING: {
		if (!m_isServerAuthorized.load(std::memory_order_acquire)) {
			ClientLog(LogLevel::Warning, "ACTION_SET_VEHICLE_HANDLING: rejected (server not authorized)");
			return false;
		}
		...
		if (!bs->Read(vehicleId) || !bs->Read(count)) {
			ClientLog(LogLevel::Error, "ACTION_SET_VEHICLE_HANDLING: Failed to read vehicleId/count");
			return false;
		}

		ClientLog(LogLevel::Debug, std::format("ACTION_SET_VEHICLE_HANDLING: vehicleId={}, count={}", vehicleId, count));
	...
	case ACTION_SET_VEHICLE_DOOR_STATE: {
		...
		if (!bs->Read(vehicleId) || !bs->Read(doorId) || !bs->Read(missing)) {
			ClientLog(LogLevel::Error, "ACTION_SET_VEHICLE_DOOR_STATE: Failed to read vehicleId/doorId/missing");
			return false;
		}

		ClientLog(LogLevel::Debug, std::format("ACTION_SET_VEHICLE_DOOR_STATE: vehicleId={}, doorId={}, missing={}", vehicleId, doorId, missing));
	...
	case ACTION_RESET_VEHICLE: {
		...
		if (!bs->Read(vehicleId)) {
			ClientLog(LogLevel::Error, "ACTION_RESET_VEHICLE: Failed to read vehicleId");
			return false;
		}
		ClientLog(LogLevel::Info, std::format("ACTION_RESET_VEHICLE: vehicleId={}", vehicleId));
	...
	case ACTION_SET_MODEL_HANDLING: {
		if (!m_isServerAuthorized.load(std::memory_order_acquire)) {
			ClientLog(LogLevel::Warning, "ACTION_SET_MODEL_HANDLING: rejected (server not authorized)");
			return false;
		}
		uint16_t modelId;
		uint8_t count;
		if (!bs->Read(modelId) || !bs->Read(count)) {
			ClientLog(LogLevel::Error, "ACTION_SET_MODEL_HANDLING: Failed to read modelId/count");
			return false;
		}
		ClientLog(LogLevel::Debug, std::format("ACTION_SET_MODEL_HANDLING: modelId={}, count={}", modelId, count));
		auto entries = ParseAttribEntries(count, bs);
	...
	case ACTION_RESET_MODEL: {
		uint16_t modelId;
		if (!bs->Read(modelId)) {
			ClientLog(LogLevel::Error, "ACTION_RESET_MODEL: Failed to read modelId");
			return false;
		}
		ClientLog(LogLevel::Info, std::format("ACTION_RESET_MODEL: modelId={}", modelId));
		ResetModel(modelId);
		return true;
	}
	...
	case ACTION_SET_PLAYER_HANDLING: {
		if (!m_isServerAuthorized.load(std::memory_order_acquire)) {
			ClientLog(LogLevel::Warning, "ACTION_SET_PLAYER_HANDLING: rejected (server not authorized)");
			return false;
		}
		uint16_t playerId;
		uint8_t count;
		if (!bs->Read(playerId) || !bs->Read(count)) {
			ClientLog(LogLevel::Error, "ACTION_SET_PLAYER_HANDLING: Failed to read playerId/count");
			return false;
		}
		ClientLog(LogLevel::Debug, std::format("ACTION_SET_PLAYER_HANDLING: playerId={}, count={}", playerId, count));
		auto entries = ParseAttribEntries(count, bs);
	...
	case ACTION_RESET_PLAYER_HANDLING: {
		uint16_t playerId;
		if (!bs->Read(playerId)) {
			ClientLog(LogLevel::Error, "ACTION_RESET_PLAYER_HANDLING: Failed to read playerId");
			return false;
		}
		if (!m_isServerAuthorized.load(std::memory_order_acquire)) {
			ClientLog(LogLevel::Warning, "ACTION_RESET_PLAYER_HANDLING: rejected (server not authorized)");
			return false;
		}
		ClientLog(LogLevel::Info, std::format("ACTION_RESET_PLAYER_HANDLING: playerId={}", playerId));
	...
	case ACTION_GET_VEHICLE_HANDLING: {
		uint16_t vehicleId;
		if (!bs->Read(vehicleId)) {
			ClientLog(LogLevel::Error, "ACTION_GET_VEHICLE_HANDLING: Failed to read vehicleId");
			return false;
		}
		ClientLog(LogLevel::Debug, std::format("ACTION_GET_VEHICLE_HANDLING: vehicleId={}", vehicleId));
	...
	case ACTION_GET_MODEL_HANDLING: {
		uint16_t modelId;
		if (!bs->Read(modelId)) {
			ClientLog(LogLevel::Error, "ACTION_GET_MODEL_HANDLING: Failed to read modelId");
			return false;
		}
		ClientLog(LogLevel::Debug, std::format("ACTION_GET_MODEL_HANDLING: modelId={}", modelId));
		tHandlingData* data = nullptr;
		auto it = m_modelHandlings.find(modelId);
		if (it != m_modelHandlings.end()) {
			data = it->second.get();
	...
	case ACTION_GET_PLAYER_HANDLING: {
		uint16_t playerId;
		if (!bs->Read(playerId)) {
			ClientLog(LogLevel::Error, "ACTION_GET_PLAYER_HANDLING: Failed to read playerId");
			return false;
		}
		ClientLog(LogLevel::Debug, std::format("ACTION_GET_PLAYER_HANDLING: playerId={}", playerId));
		tHandlingData* data = nullptr;
		auto it = m_playerHandlings.find(playerId);
		if (it != m_playerHandlings.end()) {
			data = it->second.get();
	...
	case ACTION_RESET_ALL: {
		ClientLog(LogLevel::Info, "ACTION_RESET_ALL: Resetting all vehicles, models, and players");
	...
	case ACTION_ASSET_BEGIN: {
		ClientLog(LogLevel::Debug, "ACTION_ASSET_BEGIN");
	...
	case ACTION_ASSET_END: {
		ClientLog(LogLevel::Debug, "ACTION_ASSET_END");
		ModelTransferClient::Instance().OnTransferEnd(bs);
		return true;
	}
	case ACTION_ASSET_CANCEL:
	case ACTION_ASSET_REJECTED: {
		ClientLog(LogLevel::Warning, std::format("ACTION_ASSET_CANCEL / REJECTED: action={}", static_cast<int>(action)));
	...
	case static_cast<CustomVehAction>(CustomVeh::Protocol::Action::CustomVehicleUnbind): {
		...
		if (bs->Read(reinterpret_cast<char*>(&unbinding), sizeof(unbinding))) {
			ClientLog(LogLevel::Info, std::format("CustomVehicleUnbind: vehId={}", unbinding.sampVehicleId));
			...
			ClientLog(LogLevel::Error, "CustomVehicleUnbind: Failed to read unbinding");
			return false;
		}
		return true;
	}
	...
	case static_cast<CustomVehAction>(CustomVeh::Protocol::Action::SetVehicleExtras): {
		...
		if (bs->Read(reinterpret_cast<char*>(&extras), sizeof(extras))) {
			ClientLog(LogLevel::Info, std::format("SetVehicleExtras: vehId={}, mask=0x{:X}", extras.sampVehicleId, extras.extrasMask));
	...
	case static_cast<CustomVehAction>(CustomVeh::Protocol::Action::SetVehiclePaintjob): {
		...
		if (bs->Read(reinterpret_cast<char*>(&pj), sizeof(pj))) {
			ClientLog(LogLevel::Info, std::format("SetVehiclePaintjob: vehId={}, paintjob={}", pj.sampVehicleId, static_cast<int>(pj.paintjobIndex)));
