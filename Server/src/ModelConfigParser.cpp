...
uint8_t ModelConfigParser::ParseVehicleClass(const std::string& str)
{
	...
	if (s == "poorfamily") return 1;
	if (s == "richfamily") return 2;
	if (s == "executive") return 3;
	if (s == "worker") return 4;
	if (s == "special") return 5;
	if (s == "big") return 6;
	if (s == "taxi") return 7;
	if (s == "moped") return 8;
	if (s == "motorbike") return 9;
	if (s == "leisureboat") return 10;
	if (s == "workerboat") return 11;
	if (s == "bicycle") return 12;
	if (s == "ignore") return 13;
	...
	while (std::getline(ss, rawLine))
	{
		...
		if (currentSection == "model" || currentSection == "general")
		{
			if (lowerKey == "name" || lowerKey == "friendlyname") {
				...
				if (id > 0) outConfig.visualBase = static_cast<uint32_t>(id);
			} else if (lowerKey == "audiobase" || lowerKey == "audiobasemodel") {
				int id = ResolveVehicleModelId(val);
				if (id > 0) outConfig.audioBase = static_cast<uint32_t>(id);
			} else if (lowerKey == "handlingbase" || lowerKey == "handlingbasemodel") {
				int id = ResolveVehicleModelId(val);
				if (id > 0) outConfig.handlingBase = static_cast<uint32_t>(id);
